...
# معلومات المحافظ وتفاصيل الدفع المحدثة
SHAM_CASH_WALLET = '02d28a07292f2a11f12e0d8e2bd08dd1'
BINANCE_PAY_ID = 'User-eb517'  # المستخرج من صورة الباركود المرفقة[span_1](start_span)[span_1](end_span)
...
# --- (أقسام الألعاب والخدمات تظل كما هي في نسختك الكاملة) ---
...
# استقبال مدخلات العميل وإرسال الإشعار للإدارة مع عرض كافة وسائل الدفع والباركودات
def process_user_order(message, service_name):
  ...
  try:
    ...
    # إرسال باركود بينانس المحدث إن وجد
    if os.path.exists('binance.jpg'):
      with open('binance.jpg', 'rb') as photo2:
        bot.send_photo(
            message.chat.id,
            photo2,
            caption='🟡 **باركود بينانس (Binance - User-eb517):**',
            parse_mode='Markdown',
        )

    # إرسال الرسالة النصية التوجيهية وتفاصيل المحافظ كاملة
    bot.send_message(
        message.chat.id,
        caption_text,
        reply_markup=get_persistent_keyboard(),
